...
function process(filePath) {
    ...
    const regex = /local\s+(\w+)\s*=\s*TS\.import\(script,\s*game:GetService\("ReplicatedStorage"\),\s*"TS",\s*"([^"]+)"(?:,\s*"([^"]+)")?\)(?:\.(\w+))?/g;
    
    content = content.replace(regex, (match, varName, baseImport, subImport, propName) => {
        const importPath = subImport ? path.join(baseImport, subImport) : baseImport;
        if (!seenImports.has(match)) {
...
const source = path.join(outDir, 'client', 'source.client.luau');
...
--[[
    ...
    Message me (@mr.suno) on Discord for any questions, concerns or issues regarding Starry.
